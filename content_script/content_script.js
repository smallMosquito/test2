...
var _contentScript = {
    ...
    init: function () {
        ...
        document.addEventListener('click', function (event) {
            ...
            _contentScript.deleteHighlight(highlightId);
        ...
        document.onkeydown=function(e)
        {
            ...
            var range = _xpath.createXPathRangeFromRange(_contentScript.getSelectionRange());
            if(currKey>=49 && currKey<=49+9)
            {
                var className = _storage.highlightDefinitions.defaultHighlightDefintions[(currKey - 49)].className;
                var ret = !!_contentScript.createHighlight(range, "highlight" + (_idIndex++), className);
            }
            else
            {
                return false;
